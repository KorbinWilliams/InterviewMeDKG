		...
		, methods: {
			register () {
				...
				if (password !== this.creds.repeatPass) {
					...
					console.log ('bad pass bitch, watch it or we gon dance on the blacktop playboy bitch');
					return
					...
					return
				} else if (!password.match(/[0-9]/g)||!password.match(/[a-z]/g)||!password.match(/[A-Z]/g)) {
					this.$notify({
						group: 'login'
						, title: 'Bad Password'
						, text:
